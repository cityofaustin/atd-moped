...
CREATE OR REPLACE VIEW component_arcgis_online_view AS (
    WITH work_types AS (
        ...
        SELECT
            ...
            string_agg(mwt.name, ', ') AS work_types
        FROM
...
subcomponents AS (
        ...
        SELECT
            project_component_id,
            string_agg(ms.subcomponent_name, ', ') AS subcomponents
        FROM
...
component_tags AS (
        ...
        SELECT
            project_component_id,
            string_agg(mct.type || ' - ' || mct.name, ', ') AS component_tags
        FROM
