...
WITH area_project_buffers AS (
    SELECT
        projects.project_id,
        st_buffer(
            st_transform(projects.geography::geometry, 2277), 100::double precision
        ) AS buffer_geom
    FROM project_geography projects
    INNER JOIN moped_project mp ON projects.project_id = mp.project_id
    WHERE
        mp.is_deleted = false
        AND (
            st_geometrytype(projects.geography::geometry)
            = any(ARRAY['ST_MultiPoint'::text, 'ST_MultiLineString'::text])
        )
),
...
area_district_aggregates AS (
    SELECT
        ...
        sum(
            st_area(
                st_intersection(
                    project_buffers.dissolved_buffer_geom,
    ...
    INNER JOIN
        layer_council_district districts
        ON
            st_intersects(
                project_buffers.dissolved_buffer_geom,
                st_transform(districts.geography::geometry, 2277)
...
SELECT
    ...
    round(
        (
            ada.total_area_in_district_sq_ft
            * 100::double precision
            / nullif(pt.total_area, 0::double precision)
        )::numeric,
        2
    )                       AS percentage_of_total
FROM area_district_aggregates ada
INNER JOIN project_totals pt ON ada.project_id = pt.project_id
