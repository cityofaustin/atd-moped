...
CREATE OR REPLACE VIEW project_list_view AS  WITH project_person_list_lookup AS (
         ...
         SELECT mpf.project_id,
            ...
            string_agg(DISTINCT
                ...
                CASE
                    ...
                    WHEN mfp.funding_program_name IS NOT NULL THEN mfp.funding_program_name
                    ELSE NULL::text
                END)) AS funding_source_and_program_names
         ...
         WITH project_council_district_map AS (
                 SELECT DISTINCT moped_project.project_id,
                 ...
                 SELECT parent_projects.project_id,
 ...
 SELECT mp.project_id,
    ...
    ( SELECT moped_project.project_name_full
           FROM moped_project
          WHERE moped_project.project_id = mp.parent_project_id) AS parent_project_name,
    ...
    ( SELECT json_agg(json_build_object('signal_id', feature_signals.signal_id, 'knack_id', feature_signals.knack_id, 'location_name', feature_signals.location_name, 'signal_type', feature_signals.signal_type, 'id', feature_signals.id)) AS json_agg
          ...
          WHERE true AND components.is_deleted = false AND components.project_id = mp.project_id AND feature_signals.signal_id IS NOT NULL AND feature_signals.is_deleted = false) AS project_feature,
    fsl.funding_source_name,
    fsl.funding_program_names,
    fsl.funding_source_and_program_names,
    ( SELECT min(phases.phase_start) AS min
           FROM moped_proj_phases phases
          WHERE true AND phases.project_id = mp.project_id AND phases.phase_id = 9 AND phases.is_deleted = false) AS construction_start_date,
   ...
   FROM moped_project mp
     ...
     LEFT JOIN LATERAL ( SELECT mpn.project_note,
