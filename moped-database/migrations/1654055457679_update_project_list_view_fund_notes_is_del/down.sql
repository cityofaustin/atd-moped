CREATE OR REPLACE VIEW "public"."project_list_view" AS 
 ...
 SELECT mp.project_uuid,
    ...
    COALESCE(( SELECT json_agg(features.feature) AS json_agg
           FROM (moped_proj_components components
             LEFT JOIN moped_proj_features features ON ((features.project_component_id = components.project_component_id)))
          WHERE (true AND (components.status_id = 1) AND (components.project_id = mp.project_id))), '{}'::json) AS project_feature,
    ...
    ( SELECT phases.phase_start
          ...
          WHERE (true AND (phases.project_id = mp.project_id) AND (phases.phase_name = 'construction'::text))
          ORDER BY phases.date_added DESC
         LIMIT 1) AS construction_start_date,
    ( SELECT phases.phase_end
           FROM moped_proj_phases phases
          WHERE (true AND (phases.project_id = mp.project_id) AND (phases.phase_name = 'complete'::text))
          ORDER BY phases.date_added DESC
