...
-- Refactor substantial_comletion_date and add substantial_completion_date_estimated to project_list_view
...
min_confirmed_phase_dates AS (
    WITH min_dates AS (
        ...
        LEFT JOIN moped_phases
            ON phases.phase_id = moped_phases.phase_id WHERE true
...
min_estimated_phase_dates AS (
    WITH min_dates AS (
        ...
        LEFT JOIN moped_phases
            ON phases.phase_id = moped_phases.phase_id WHERE true
            AND phases.phase_start IS NOT null
            AND phases.is_phase_start_confirmed = false
            AND moped_phases.phase_name_simple = 'Complete'::text
            AND phases.is_deleted = false
        GROUP BY
            phases.project_id
        ...
        LEFT JOIN moped_phases
            ON phases.phase_id = moped_phases.phase_id WHERE true
            AND phases.phase_end IS NOT null
            AND phases.is_phase_end_confirmed = false
            AND moped_phases.phase_name_simple = 'Complete'::text
            AND phases.is_deleted = false
        GROUP BY
            phases.project_id
...
SELECT
    ...
    CASE WHEN mcpd.min_phase_date IS NOT null THEN null ELSE mepd.min_phase_date END AS substantial_completion_date_estimated,
    (
...
-- Reference substantial_completion_date_estimated from project_list_view
...
council_districts AS (
    SELECT
        ...
        string_agg(DISTINCT lpad(features_council_districts.council_district_id::text, 2, '0'), ', '::text) AS council_districts_searchable
...
related_projects AS (
    SELECT
        pmp.project_id,
        concat_ws(', ', pmp.project_id, string_agg(cmp.project_id::text, ', '::text)) AS related_project_ids_with_self,
...
SELECT
    ...
    plv.substantial_completion_date_estimated AS substantial_completion_date_estimated,
    mpc.srts_id,
