...
-- OPTIMIZATION 1: Rewrite project_district_association
project_council_district_map AS (
    SELECT DISTINCT
        components.project_id,
        districts.council_district_id
    FROM moped_proj_components AS components
    INNER JOIN features ON components.project_component_id = features.component_id AND features.is_deleted IS false
    INNER JOIN features_council_districts AS districts ON features.id = districts.feature_id
...
project_districts AS (
    SELECT
        project_id,
        jsonb_agg(DISTINCT council_district_id) FILTER (WHERE council_district_id IS NOT null) AS project_council_districts
...
child_project_districts AS (
    ...
    FROM moped_project AS p
    INNER JOIN moped_project AS c ON p.project_id = c.parent_project_id AND c.is_deleted = false
    INNER JOIN project_council_district_map AS d ON c.project_id = d.project_id
        ...
        CASE
            WHEN cd.child_project_districts IS NOT null
                THEN (
                    ...
                    FROM jsonb_array_elements(pd.project_council_districts || cd.child_project_districts) AS elem
                )
            ELSE pd.project_council_districts
        END AS project_and_child_project_council_districts
    FROM moped_project AS p
    LEFT JOIN project_districts AS pd ON p.project_id = pd.project_id
    LEFT JOIN child_project_districts AS cd ON p.project_id = cd.project_id
