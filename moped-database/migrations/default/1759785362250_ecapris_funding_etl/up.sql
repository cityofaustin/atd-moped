...
-- Add unique constraint on (fao_id, project_id, is_synced_from_ecapris) in moped_proj_funding table so we prevent duplicates
ALTER TABLE moped_proj_funding
ADD CONSTRAINT moped_proj_funding_unique_fao_id_project_id UNIQUE (fao_id, project_id, is_synced_from_ecapris),
ADD COLUMN is_synced_from_ecapris BOOLEAN DEFAULT FALSE NOT NULL;

COMMENT ON CONSTRAINT moped_proj_funding_unique_fao_id_project_id ON moped_proj_funding IS 'Prevents duplicate funding records from being synced from eCAPRIS.';
