...
SELECT
    ...
    (
        SELECT string_agg(text_rows, ', ') 
        FROM jsonb_array_elements_text(districts.project_council_districts) as text_rows
    ) AS project_council_districts_str,
    districts.project_and_child_project_council_districts,
    (
        SELECT string_agg(text_rows, ', ') 
        FROM jsonb_array_elements_text(districts.project_and_child_project_council_districts) as text_rows
    ) AS project_and_child_project_council_districts_str,
