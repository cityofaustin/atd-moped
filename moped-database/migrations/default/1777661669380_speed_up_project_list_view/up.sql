-- Optimizes project_list_view: replace per-row correlated subqueries with one-time CTE aggregations,
-- scan "Complete" phase rows once for substantial completion dates, and simplify funding aggregates.
--
-- This migration does NOT create indexes. Consider these for heavy use of this view (validate with EXPLAIN ANALYZE):
--
-- | Area | Suggestion | Notes |
-- | Work activity rollup | CREATE INDEX ON moped_proj_work_activity (project_id) WHERE is_deleted = false | Feeds work_activities GROUP BY |
-- | Signals on components | CREATE INDEX ON feature_signals (component_id) WHERE is_deleted = false | project_signals_feature join path |
-- | Partners | CREATE INDEX ON moped_proj_partners (project_id) WHERE is_deleted = false | project_partners_lookup |
-- | Construction start | CREATE INDEX ON moped_proj_phases (project_id, phase_id) WHERE is_deleted = false | phase_id = 9 filter |
--
-- Already common in repo: moped_proj_phases (project_id), moped_proj_personnel (project_id),
...
project_partners_lookup AS (
    ...
    JOIN moped_entity me2 ON mpp2.entity_id = me2.entity_id
...
funding_filtered AS (
    SELECT
        ...
        CASE
            ...
            ELSE null::text
        END AS source_program_display
    FROM combined_project_funding_view cfv
    JOIN moped_project proj ON cfv.project_id = proj.project_id
    WHERE cfv.is_synced_from_ecapris = false
       OR (
            proj.should_sync_ecapris_funding = true
            AND proj.ecapris_subproject_id IS NOT null
...
funding_sources_lookup AS (
    SELECT
        project_id,
        string_agg(DISTINCT source_name, ', '::text ORDER BY source_name) AS funding_source_name,
        string_agg(DISTINCT program_name, ', '::text ORDER BY program_name) AS funding_program_names,
        string_agg(DISTINCT source_program_display, ', '::text ORDER BY source_program_display) AS funding_source_and_program_names
    FROM funding_filtered
    GROUP BY project_id
),
...
complete_phase_project_rows AS (
    SELECT
        phases.project_id,
        phases.phase_start,
        phases.phase_end,
        phases.is_phase_start_confirmed,
        phases.is_phase_end_confirmed
    FROM moped_proj_phases phases
    LEFT JOIN moped_phases ON phases.phase_id = moped_phases.phase_id
    WHERE phases.is_deleted = false
      AND moped_phases.phase_name_simple = 'Complete'::text
),

min_confirmed_phase_dates AS (
    SELECT
        d.project_id,
        min(d.dt) AS min_phase_date
    FROM (
        SELECT project_id, phase_start AS dt
        FROM complete_phase_project_rows
        WHERE phase_start IS NOT null
          AND is_phase_start_confirmed = true
        UNION ALL
        SELECT project_id, phase_end AS dt
        FROM complete_phase_project_rows
        WHERE phase_end IS NOT null
          AND is_phase_end_confirmed = true
    ) d
...
min_estimated_phase_dates AS (
    ...
    FROM (
        ...
        FROM complete_phase_project_rows
        WHERE phase_start IS NOT null
          AND is_phase_start_confirmed = false
        UNION ALL
        SELECT project_id, phase_end AS dt
        FROM complete_phase_project_rows
        WHERE phase_end IS NOT null
          AND is_phase_end_confirmed = false
    ) d
    GROUP BY d.project_id
),
...
SELECT
    ...
    mp_parent.project_name_full AS parent_project_name,
    cpl.children_project_ids,
    ppl.project_partners,
    psf.project_feature,
    fsl.funding_source_name,
    fsl.funding_program_names,
    fsl.funding_source_and_program_names,
    csd.construction_start_date,
...
LEFT JOIN construction_start_dates csd ON mp.project_id = csd.project_id
