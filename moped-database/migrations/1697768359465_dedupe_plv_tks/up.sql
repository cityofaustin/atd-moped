...
WITH project_person_list_lookup AS (
    SELECT
        mpp.project_id,
        string_agg(DISTINCT concat(mu.first_name,
                ' ',
                mu.last_name,
                ':',
                mpr.project_role_name),
            ','::text) AS project_team_members
    FROM
        moped_proj_personnel mpp
...
funding_sources_lookup AS (
    SELECT
        mpf_1.project_id,
        string_agg(mfs.funding_source_name,
            ', '::text) AS funding_source_name
    FROM
        moped_proj_funding mpf_1
    LEFT JOIN moped_fund_sources mfs ON mpf_1.funding_source_id = mfs.funding_source_id
WHERE
    mpf_1.is_deleted = FALSE
GROUP BY
    mpf_1.project_id
),
project_type_lookup AS (
    SELECT
        mpt.project_id,
        string_agg(mt.type_name,
            ', '::text) AS type_name
    FROM
        moped_project_types mpt
    LEFT JOIN moped_types mt ON mpt.project_type_id = mt.type_id
        AND mpt.is_deleted = FALSE
    GROUP BY
        mpt.project_id
),
...
SELECT
    ...
    (
    SELECT
        project_name FROM moped_project WHERE project_id = mp.parent_project_id) AS parent_project_name,
    cpl.children_project_ids,
    string_agg(DISTINCT me2.entity_name, ', '::text) AS project_partner,
    (
        SELECT
            JSON_AGG(json_build_object('signal_id', feature_signals.signal_id, 'knack_id', feature_signals.knack_id, 'location_name', feature_signals.location_name, 'signal_type', feature_signals.signal_type, 'id', feature_signals.id))
        FROM
            moped_proj_components components
    LEFT JOIN feature_signals ON (feature_signals.component_id = components.project_component_id)
WHERE
    TRUE
    AND components.is_deleted = FALSE
    AND components.project_id = mp.project_id
    AND feature_signals.signal_id IS NOT NULL
    AND feature_signals.is_deleted = FALSE) AS project_feature, fsl.funding_source_name, ptl.type_name, ( -- get the date of the construction phase with the earliest start date
        SELECT
            min(phases.phase_start)
        FROM
        ...
        WHERE
            ...
            AND phases.is_deleted = FALSE) AS construction_start_date, ( -- get the date of the completion phase with the latest end date
        SELECT
            max(phases.phase_end)
        FROM
            moped_proj_phases phases
        WHERE
            TRUE
            ...
            AND phases.is_deleted = FALSE) AS completion_end_date, ( -- get me a list of the inspectors for this project
        SELECT
            string_agg(concat(users.first_name, ' ', users.last_name), ', '::text) AS string_agg
        FROM
            moped_proj_personnel mpp
    ...
    ( -- get me all of the tags added to a project
        SELECT
            string_agg(tags.name, ', '::text) AS string_agg
        FROM
            moped_proj_tags ptags
...
FROM
    ...
    LEFT JOIN moped_proj_partners mpp2 ON mp.project_id = mpp2.project_id
        AND mpp2.is_deleted = FALSE
    ...
    LEFT JOIN LATERAL (
        ...
        WHERE
            mpn.project_id = mp.project_id
            AND mpn.project_note_type = 2
            AND mpn.is_deleted = FALSE
        ORDER BY
            mpn.date_created DESC
        LIMIT 1) AS proj_notes ON TRUE
WHERE
    mp.is_deleted = FALSE
GROUP BY
    mp.project_id,
