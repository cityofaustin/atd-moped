...
SELECT
    ...
    true                                                           AS is_editable,
    moped_note_types.slug = 'status_update'::text                  AS is_status_update,
...
SELECT
    ...
    COALESCE(
        ...
        CASE
            WHEN
                ecapris_subproject_statuses.reviewed_by_name ~~ '%,%'::text
                THEN
                    (
                        TRIM(
                            BOTH FROM SPLIT_PART(
                                ecapris_subproject_statuses.reviewed_by_name, ','::text, 2
                            )
                        )
                        || ' '::text
                    )
                    || TRIM(
                        BOTH FROM SPLIT_PART(
                            ecapris_subproject_statuses.reviewed_by_name, ','::text, 1
                        )
                    )
