...
-- change column types to dates
ALTER TABLE moped_proj_phases
  ALTER COLUMN phase_start TYPE date,
  ALTER COLUMN phase_end TYPE date;

-- disable event triggers and assign UTC  to all existing dates
...
FROM (
    SELECT
        project_id,
        phase_start AT TIME ZONE 'America/Chicago' AT TIME ZONE 'UTC' AS phase_start,
        phase_end AT TIME ZONE 'America/Chicago' AT TIME ZONE 'UTC' AS phase_end
    FROM
        moped_proj_phases) AS dates_to_fix
WHERE
    moped_proj_phases.project_id = dates_to_fix.project_id;

-- replace timezone of component completion dates which were imported from Access DB as UTC
-- there is no way to undo this operation:
-- UPDATE
--     moped_proj_components
-- SET
--     completion_date = dates_to_fix.completion_date
-- FROM (
--     SELECT
--         project_component_id,
--         completion_date AT TIME ZONE 'UTC' AT TIME ZONE 'America/Chicago' AS completion_date
--     FROM
--         moped_proj_components
--     WHERE
--         -- if the timestamp is 00:00:00 it was imported incorrectly as UTC time
--         completion_date IS NOT NULL
--         AND extract(hour FROM completion_date) = 0) as dates_to_fix
-- WHERE
--     moped_proj_components.project_component_id = dates_to_fix.project_component_id;

-- rebuild dropped views to version 1704744986000_substantial_completion_date
