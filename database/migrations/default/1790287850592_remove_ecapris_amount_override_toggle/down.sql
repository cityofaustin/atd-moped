...
-- Restore inherited eCAPRIS values into moped_proj_funding and turn the amount toggle back on where inherited
UPDATE moped_proj_funding
SET
    should_use_ecapris_amount = moped_proj_funding.funding_amount IS NULL,
    funding_amount = COALESCE(moped_proj_funding.funding_amount, ecapris_subproject_funding.app),
    funding_source_id = COALESCE(moped_proj_funding.funding_source_id, ecapris_subproject_funding.funding_source_id),
    funding_program_id = COALESCE(moped_proj_funding.funding_program_id, ecapris_subproject_funding.funding_program_id)
...
UNION ALL
SELECT
    (('ecapris_'::text || ecapris_subproject_funding.id) || '_moped_'::text)
    || moped_project.project_id              AS id,
...
LEFT JOIN
    moped_fund_sources
    ON ecapris_subproject_funding.funding_source_id = moped_fund_sources.funding_source_id
LEFT JOIN
    moped_fund_programs
    ON ecapris_subproject_funding.funding_program_id = moped_fund_programs.funding_program_id
INNER JOIN
    moped_project
    ON ecapris_subproject_funding.ecapris_subproject_id = moped_project.ecapris_subproject_id
WHERE NOT (EXISTS (
    SELECT 1
    FROM moped_proj_funding
    WHERE
        moped_proj_funding.fdu = ecapris_subproject_funding.fdu
