...
-- Disable triggers so these updates don't write activity log entries or touch audit fields,
-- since the values displayed in the UI and downstream reporting do not change
DO $$
BEGIN
...
-- Null out amounts that currently display the eCAPRIS amount (toggle on) or are redundant with it
-- so the eCAPRIS amount is inherited through the combined view
UPDATE moped_proj_funding
SET funding_amount = NULL
FROM ecapris_subproject_funding
WHERE
    moped_proj_funding.ecapris_funding_id = ecapris_subproject_funding.fao_id
    AND moped_proj_funding.is_deleted = FALSE
    AND moped_proj_funding.funding_amount IS NOT NULL
    AND (
        moped_proj_funding.should_use_ecapris_amount = TRUE
        OR moped_proj_funding.funding_amount = ecapris_subproject_funding.app
    );

-- Null out sources and programs that are redundant with eCAPRIS values
UPDATE moped_proj_funding
...
WHERE
    moped_proj_funding.ecapris_funding_id = ecapris_subproject_funding.fao_id
    AND moped_proj_funding.is_deleted = FALSE
    AND moped_proj_funding.funding_source_id = ecapris_subproject_funding.funding_source_id;

UPDATE moped_proj_funding
SET funding_program_id = NULL
FROM ecapris_subproject_funding
WHERE
    moped_proj_funding.ecapris_funding_id = ecapris_subproject_funding.fao_id
    AND moped_proj_funding.is_deleted = FALSE
    AND moped_proj_funding.funding_program_id = ecapris_subproject_funding.funding_program_id;
...
ALTER TABLE moped_proj_funding DROP COLUMN should_use_ecapris_amount;

-- Values stored in moped_proj_funding are overrides; when null, the eCAPRIS value is inherited
CREATE OR REPLACE VIEW combined_project_funding_view AS SELECT
    ...
    COALESCE(moped_proj_funding.funding_amount, ecapris_subproject_funding.app) AS amount,
    moped_proj_funding.funding_description AS description,
    moped_fund_sources.funding_source_name AS source_name,
    COALESCE(moped_proj_funding.funding_source_id, ecapris_subproject_funding.funding_source_id) AS funding_source_id,
    ...
    COALESCE(moped_proj_funding.funding_program_id, ecapris_subproject_funding.funding_program_id) AS funding_program_id,
...
LEFT JOIN moped_fund_sources ON COALESCE(moped_proj_funding.funding_source_id, ecapris_subproject_funding.funding_source_id) = moped_fund_sources.funding_source_id
LEFT JOIN moped_fund_programs ON COALESCE(moped_proj_funding.funding_program_id, ecapris_subproject_funding.funding_program_id) = moped_fund_programs.funding_program_id
...
INNER JOIN
    ...
    ON moped_project.project_id = combined_project_funding_view.project_id
WHERE
    moped_project.is_deleted = false AND (combined_project_funding_view.is_synced_from_ecapris = false OR moped_project.should_sync_ecapris_funding = true AND moped_project.ecapris_subproject_id IS NOT null AND combined_project_funding_view.is_synced_from_ecapris = true);
...
WITH project_person_list_lookup AS (
    ...
    INNER JOIN moped_users mu ON mpp.user_id = mu.user_id
    INNER JOIN
        moped_proj_personnel_roles mppr
        ON mpp.project_personnel_id = mppr.project_personnel_id
    INNER JOIN moped_project_roles mpr ON mppr.project_role_id = mpr.project_role_id
...
funding_sources_lookup AS (
    SELECT
        ...
        string_agg(
            ...
            CASE
                WHEN
                    cfv.source_name IS NOT null AND cfv.program_name IS NOT null
                    THEN concat(cfv.source_name, ' - ', cfv.program_name)
...
work_activities AS (
    SELECT
        mpwa.project_id,
        string_agg(
            task_order_objects.task_order_object ->> 'display_name'::text, ', '::text
        ) AS task_order_names,
        string_agg(
            task_order_objects.task_order_object ->> 'task_order'::text, ', '::text
        ...
        string_agg(
            DISTINCT mpwa.workgroup_contractor, ', '::text
        ) AS workgroup_contractors,
        string_agg(
            mpwa.contract_number, ', '::text
        ) AS contract_numbers
    FROM moped_proj_work_activity mpwa
    LEFT JOIN LATERAL
        jsonb_array_elements(mpwa.task_orders) task_order_objects (task_order_object)
        ON true
...
project_district_association AS (
    WITH project_council_district_map AS (
        ...
        FROM moped_proj_components mpc
        INNER JOIN features f ON mpc.project_component_id = f.component_id
        INNER JOIN features_council_districts fcd ON f.id = fcd.feature_id
...
parent_child_project_map AS (
        ...
        FROM moped_project parent_projects
        INNER JOIN
            moped_project child_projects
            ON parent_projects.project_id = child_projects.parent_project_id
        WHERE child_projects.is_deleted = false
    )

    SELECT
        projects.project_id,
        jsonb_agg(DISTINCT project_districts.council_district_id) FILTER (
            WHERE project_districts.council_district_id IS NOT null
        ) AS project_council_districts,
        jsonb_agg(DISTINCT project_and_children_districts.council_district_id) FILTER (
            WHERE project_and_children_districts.council_district_id IS NOT null
        ) AS project_and_child_project_council_districts,
        nullif(
            array_to_string(array_agg(
                DISTINCT project_districts.council_district_id
                ORDER BY project_districts.council_district_id
            ), ', '::text), ''::text
        ) AS project_council_districts_string,
        nullif(
            array_to_string(array_agg(
                DISTINCT project_and_children_districts.council_district_id
                ORDER BY project_and_children_districts.council_district_id
            ), ', '::text), ''::text
        ) AS project_and_child_project_council_districts_string
    FROM moped_project projects
    LEFT JOIN
        project_council_district_map project_districts
        ON projects.project_id = project_districts.project_id
    LEFT JOIN
        parent_child_project_map project_family
        ON projects.project_id = project_family.project_id
    LEFT JOIN
        project_council_district_map project_and_children_districts
        ON project_family.self_and_children_project_id = project_and_children_districts.project_id
...
phase_date_inputs AS (
    ...
    INNER JOIN moped_phases ON phases.phase_id = moped_phases.phase_id
...
project_component_work_types AS (
    ...
    LEFT JOIN
        ...
        ON mpc.project_component_id = mpcwt.project_component_id
...
project_partners_lookup AS (
    SELECT
        mpp.project_id,
        string_agg(DISTINCT me_1.entity_name, ', '::text) AS project_partners
    FROM moped_proj_partners mpp
    LEFT JOIN moped_entity me_1 ON mpp.entity_id = me_1.entity_id
...
project_roles_lookup AS (
    SELECT
        mpp.project_id,
        string_agg(concat(users.first_name, ' ', users.last_name), ', '::text) FILTER (
            WHERE mpr.project_role_name = 'Inspector'::text
        ) AS project_inspector,
        string_agg(concat(users.first_name, ' ', users.last_name), ', '::text) FILTER (
            WHERE mpr.project_role_name = 'Designer'::text
        ) AS project_designer
    FROM moped_proj_personnel mpp
    INNER JOIN moped_users users ON mpp.user_id = users.user_id
    INNER JOIN
        moped_proj_personnel_roles mppr
        ON mpp.project_personnel_id = mppr.project_personnel_id
    INNER JOIN moped_project_roles mpr ON mppr.project_role_id = mpr.project_role_id
    WHERE
        mpp.is_deleted = false
        AND mppr.is_deleted = false
        AND (mpr.project_role_name = any(ARRAY['Inspector'::text, 'Designer'::text]))
    GROUP BY mpp.project_id
),
...
project_tags_lookup AS (
    ...
    INNER JOIN moped_tags tags ON ptags.tag_id = tags.id
...
SELECT
    ...
    'https://mobility.austin.gov/moped/projects/'::text
    || mp.parent_project_id::text                                              AS parent_project_url,
...
WHERE
    (
        ...
        OR mp.should_sync_ecapris_statuses = true
...
council_districts AS (
    SELECT
        ...
        string_agg(
            DISTINCT features_council_districts.council_district_id::text, ', '::text
        )                     AS council_districts,
        string_agg(
            DISTINCT lpad(features_council_districts.council_district_id::text, 2, '0'::text),
            ', '::text
        )                     AS council_districts_searchable
...
related_projects AS (
    SELECT
        pmp.project_id,
        concat_ws(
            ', '::text, pmp.project_id, string_agg(cmp.project_id::text, ', '::text)
        ) AS related_project_ids_with_self,
        concat_ws(
            ', '::text,
            lpad(pmp.project_id::text, 5, '0'::text),
            string_agg(lpad(cmp.project_id::text, 5, '0'::text), ', '::text)
        ) AS related_project_ids_searchable_with_self
...
earliest_active_or_construction_phase_date AS (
    ...
    WHERE
        (mp.phase_name_simple = any(ARRAY['Active'::text, 'Construction'::text]))
...
SELECT
    ...
    NOT coalesce(
        council_districts.council_districts IS null
        OR council_districts.council_districts = ''::text,
        false
    ...
    CASE
        ...
        WHEN
            mpc.phase_id IS NOT null AND mpc.completion_date IS null
    ...
    coalesce(
        mph.phase_name, current_phase.phase_name
    )                                           AS current_phase_name,
    coalesce(
        mph.phase_name_simple, current_phase.phase_name_simple
    )                                           AS current_phase_name_simple,
    ...
    to_char(
        timezone('America/Chicago'::text, plv.construction_start_date), 'YYYY-MM-DD'::text
    ...
    plv.project_url,
    (plv.project_url || '?tab=map&project_component_id='::text)
    || mpc.project_component_id::text           AS component_url,
    get_project_development_status(
    ...
    )                                           AS project_development_status,
    project_development_status_date.result      AS project_development_status_date,
    to_char(
        project_development_status_date.result, 'YYYY'::text
    )::integer                                  AS project_development_status_date_calendar_year,
    to_char(
        project_development_status_date.result, 'FMMonth YYYY'::text
    )                                           AS project_development_status_date_calendar_year_month,
    to_char(
        project_development_status_date.result, 'YYYY-MM'::text
    )                                           AS project_development_status_date_calendar_year_month_numeric,
    date_part(
        'quarter'::text, project_development_status_date.result
    )::text                                     AS project_development_status_date_calendar_year_quarter,
    CASE
        WHEN
            date_part('quarter'::text, project_development_status_date.result) = 4::double precision
            THEN (to_char(project_development_status_date.result, 'YYYY'::text)::integer + 1)::text
    ...
    CASE
        ...
        ELSE
            date_part('quarter'::text, project_development_status_date.result) + 1::double precision
