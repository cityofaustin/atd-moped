...
ALTER TABLE moped_project ALTER COLUMN ecapris_subproject_id TYPE numeric;

CREATE INDEX moped_project_ecapris_subproject_id_index ON moped_project (ecapris_subproject_id);

ALTER TABLE moped_project
    ADD COLUMN capitally_funded boolean DEFAULT FALSE;
...
SELECT
    ...
    (
        SELECT
            JSON_AGG(feature.attributes) -- this query finds any signal components and those component's features and rolls them up in a JSON blob
        FROM
            moped_proj_components components
    LEFT JOIN uniform_features feature ON (feature.component_id = components.project_component_id)
WHERE
    TRUE
    AND components.is_deleted = FALSE
    AND components.project_id = mp.project_id
    AND feature.table = 'feature_signals') AS project_feature, fsl.funding_source_name, ptl.type_name, ( -- get the most recent status_update (project note type 2)
        SELECT
            mpn.project_note
        FROM
    ...
    ( -- get me all of the contractors added to a project
        SELECT
            string_agg(contract.contractor, ', '::text) AS string_agg
        FROM
            moped_proj_contract contract
        WHERE
            1 = 1
            AND contract.is_deleted = FALSE
            AND contract.project_id = mp.project_id
        GROUP BY contract.project_id) AS contractors,
    ( -- get me all of the contract numbers added to a project
        SELECT
            string_agg(contract.contract_number, ', '::text) AS string_agg
        FROM
            moped_proj_contract contract
        WHERE
            1 = 1
...
FROM
    ...
    LEFT JOIN moped_proj_contract contracts ON (mp.project_id = contracts.project_id)
        AND contracts.is_deleted = FALSE
