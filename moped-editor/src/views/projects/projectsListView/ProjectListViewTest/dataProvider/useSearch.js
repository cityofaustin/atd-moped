...
export const getSearchValue = (column, value) => {
  ...
  const type = ProjectsListViewQueryConf.columns[column].type.toLowerCase();
  // Get the invalidValueDefault in the search config object
  const invalidValueDefault =
    ProjectsListViewQueryConf.columns[column].search?.invalidValueDefault ??
    null;
  ...
  const searchWhereString = useMemo(() => {
    if (searchTerm && searchTerm !== "") {
      ...
      const searchOperatorsAndValues = Object.keys(
        ProjectsListViewQueryConf.columns
      )
        .filter(
          (column) => ProjectsListViewQueryConf.columns[column]?.searchable
        )
        .map((column) => {
